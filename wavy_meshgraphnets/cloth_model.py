...
from wavy_meshgraphnets import common
from wavy_meshgraphnets import core_model
from wavy_meshgraphnets import normalization
...
class Model(snt.AbstractModule):
  ...
  def _build_graph(self, inputs, is_training):
    ...
    edge_norm = self._edge_normalizer(edge_features, is_training)
    node_norm = self._node_normalizer(node_features, is_training)

    mesh_edges = core_model.EdgeSet(
        name='mesh_edges',
        features=edge_norm,
        prev_features=edge_norm, 
        receivers=receivers,
        senders=senders)
    return core_model.MultiGraph(
        node_features=node_norm,
        prev_node_features=node_norm, 
